...
Switching to the mainnet is as easy as changing the `provider.parameters.payment_network` property in your [cluster yaml](/docs/ja/creators/ray/cluster-yaml):
...
Feel free to learn more about [mainnet payments and funding your Golem node](/docs/ja/creators/javascript/guides/switching-to-mainnet).
...
Ray On Golem
  ...
  Running Ray on Golem on the mainnet requires GLM and MATIC tokens on the Polygon blockchain (see: https://docs.golem.network/docs/ja/creators/ray/mainnet).
  Your wallet:
...
When handling real funds, it is generally a good idea to [back up your Ray on Golem wallet](/docs/ja/providers/wallet/backup).
...
- [hard limit](/docs/ja/creators/ray/cluster-yaml#spending-hard-limit) of spending on each Ray on Golem cluster (from `ray up` to `ray down`)
- [limit of provider prices](/docs/ja/creators/ray/cluster-yaml#maximum-provider-prices) accepted by Ray on Golem
- [choosing the cheapest providers](/docs/ja/creators/ray/cluster-yaml#choosing-the-cheapest-providers-maximum-expected-usage-cost) based on your expected cluster life duration and average load
- [network stats tool](/docs/ja/creators/ray/ray-on-golem-cli#network-stats) helping you to set price limits reasonably
...
Of course, you might want to disconnect from the Golem network for a while - read the [starting over](/docs/ja/creators/ray/troubleshooting#starting-over-with-a-clean-slate) article to learn how to do that.
