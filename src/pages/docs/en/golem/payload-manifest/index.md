...
_Computation Payload Manifest_ allows a [requestor](/docs/en/golem/overview/requestor) to define an application package _Payload_ and allows to set constraints on the computations performed on a [provider](/docs/en/golem/overview/provider) node.
...
- [Importing certificates] used to sign app authors' certificates into the provider's [keystore](/docs/en/providers/configuration/outbound#managing-your-keystore) (which allows the provider agent to verify _manifest_ signatures)
- Adding domain patterns to Provider's [domain whitelist](/docs/en/providers/configuration/outbound#listing-whitelisted-domains) (which makes the manifest [signature] optional).
...
_Computation Payload Manifest_ must follow a specific [JSON Schema](https://github.com/golemfactory/yagna-docs/blob/master/requestor-tutorials/vm-runtime/computation-payload-manifest.schema.json) ([Documentation](/docs/en/golem/payload-manifest/computation-payload-manifest.schema))
...
_Payload_ definition allows to define [GVMI images](/docs/en/creators/javascript/guides/golem-images) used by Application and supported architecture.
...
With a Computation Manifest object, [requestor](/docs/en/golem/overview/requestor) constrains themself to a certain set of allowed actions, to be negotiated with and approved by a [provider](/docs/en/golem/overview/provider).
...
{% alert level="info" %}
JS SDK users can utilize the [SDK CLI tool](https://docs.golem.network/docs/en/creators/javascript/guides/golem-sdk-cli) to create and sign manifests.
{% /alert %}
