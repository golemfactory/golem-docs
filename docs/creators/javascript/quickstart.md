...
This function gets first parameter `workContext` that is a [WorkContext](../docs/classes/task_work.WorkContext/index) object. This object allow you set the commands that you want to run in the scope of one task on one provider. So the command we would like to run on the provider is `node -v`:
